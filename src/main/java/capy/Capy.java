...
public class Capy {
    ...
    private static final String WRONG_FORMAT = "Oops! Seems like the command is in the wrong format!";
    ...
    private static void markTask(String userInput) throws CapyException {
        ...
        if (parts.length < 2) {
            ...
            throw new CapyException(WRONG_FORMAT);
        }
        try {
    ...
    private static void deleteTask(String userInput) throws CapyException {
        String[] parts = userInput.split(" ");
        if (parts.length < 2) {
            throw new CapyException(MISSING_TASK_NUMBER);
        } else if (parts.length > 2) {
